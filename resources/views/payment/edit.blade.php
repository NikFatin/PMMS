<x-app-layout>
    ...
    <form method="post" action="{{ route('payment.update', $payment->id) }}" class="mt-6 space-y-6">
        ...
        <!-- <div>
            <x-input-label for="payment_id" :value="__('Transaction ID')" />
            <x-text-input id="payment_id" name="Payment ID" type="text" class="mt-1 block w-full" required autofocus autocomplete="task" />
        ...
        <div>
            ...
            <x-text-input id="total" name="total" type="text" class="mt-1 block w-full" required autofocus autocomplete="task" :value="old('total', $payment-> total)"/>
